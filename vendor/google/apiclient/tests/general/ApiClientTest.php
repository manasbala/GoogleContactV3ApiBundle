...
class ApiClientTest extends BaseTest
{
  ...
  public function testJsonConfig()
  {
    ...
    $device =
    '{"installed":{"auth_uri":"https://accounts.google.com/o/oauth2/auth","clientSecret"'.
    ...
    ':"","clientId":"123456789.apps.googleusercontent.com","auth_provider_x509_cert_url":'.
    '"https://www.googleapis.com/oauth2/v1/certs"}}';
    $dObj = json_decode($device);
    $client->setAuthConfig($device);
    $cfg = $config->getClassConfig('Google_Auth_OAuth2');
    $this->assertEquals($cfg['clientId'], $dObj->installed->client_id);
    $this->assertEquals($cfg['clientSecret'], $dObj->installed->client_secret);
    $this->assertEquals($cfg['redirect_uri'], $dObj->installed->redirect_uris[0]);

    // Web config
    $config = new Google_Config();
    $client = new Google_Client($config);
    $web = '{"web":{"auth_uri":"https://accounts.google.com/o/oauth2/auth","clientSecret"' .
    ...
    $cfg = $config->getClassConfig('Google_Auth_OAuth2');
    $this->assertEquals($cfg['clientId'], $wObj->web->client_id);
    $this->assertEquals($cfg['clientSecret'], $wObj->web->client_secret);
  ...
  public function testIniConfig()
  {
    ...
    $this->assertEquals(
        'gjfiwnGinpena3',
        $config->getClassConfig('Google_Auth_OAuth2', 'clientSecret')
    );
